...
import sqlite3
from utils.evaluator import execute_code_safely, run_test_cases 
import re
import pandas as pd
...
# ---------- DATABASE ----------
def db():
    con = sqlite3.connect("resume_screening.db")
...
def jd_match(resume, jd):
    ...
    try:
        ...
        score = cosine_similarity(vectors)[0][1] * 900
...
def skill_score(resume, skills_str):
    ...
    score = (matched_count / len(required_skills)) * 400   # ✅ FIXED
...
def format_check(resume_text):
    ...
    if samples:
        try:
            ...
            similarity_score = similarities.max() * 800
        except Exception as e:
...
# 1. THE RBAC DECORATOR
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Check for admin_id OR user_id, and check the role
...
@admin_required # This route is now locked!
def admin_dashboard():
    # Only reachable if session['role'] == 'admin'
    return render_template('admin.html')

# 3. SETTING THE ROLE DURING LOGIN
@app.route('/admin-secure-gate') # Keep this name secret
...
def admin():
    ...
    if request.method == "POST":
        ...
        # ✅ Get job_id immediately AFTER insert
        ...
        # ✅ Process Coding CSV
        coding_file = request.files.get('coding_file')
        if coding_file:
            stream = io.StringIO(coding_file.stream.read().decode("UTF8"))
            reader = csv.DictReader(stream)

            for row in reader:
                cur.execute("""
                    INSERT INTO coding_challenges 
                    (job_id, title, description, difficulty, function_name, initial_code, test_input, expected_output)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    new_job_id,
                    row['title'],
...
def assessment(job_id):
    ...
    # Fetch the coding challenge for this job
    coding = con.execute("SELECT * FROM coding_challenges WHERE job_id = ?", (job_id,)).fetchone()
    con.close()
    
    return render_template("assessment.html", questions=questions, coding=coding, job_id=job_id)

@app.route('/coding/<int:job_id>')
def coding_page(job_id):
    con = db()
    con.row_factory = sqlite3.Row
    cur = con.cursor()
    
    # Fetch all coding challenges for this job
    challenges = cur.execute("SELECT * FROM coding_challenges WHERE job_id = ?", (job_id,)).fetchall()
    con.close()
    
    if not challenges:
        flash("No coding challenges found for this job.", "warning")
        return redirect("/")

    return render_template("coding.html", challenges=challenges, job_id=job_id)
@app.route('/admin/save_assessment', methods=['POST'])
def save_assessment():
    job_id = request.form.get('job_id')
    
    # MCQ Data
    mcq_q = request.form.get('mcq_question')
    options = {
    ...
    # Coding Data
    c_title = request.form.get('coding_title')
    c_desc = request.form.get('coding_desc')
    pub_in = request.form.get('pub_input')
    pub_out = request.form.get('pub_output')
    priv_in = request.form.get('priv_input')
    priv_out = request.form.get('priv_output')

    db = sqlite3.connect("resume_screening.db")
    cur = db.cursor()

    # Save MCQ
    cur.execute("INSERT INTO questions (job_id, question, options, correct_answer) VALUES (?, ?, ?, ?)",
                (job_id, mcq_q, json.dumps(options), correct))
    
    # Save Coding Challenge
    cur.execute("""INSERT INTO coding_challenges 
                (job_id, title, description, public_input, public_expected, private_input, private_expected) 
                VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (job_id, c_title, c_desc, pub_in, pub_out, priv_in, priv_out))
...
def bulk_upload(job_id):
    ...
    for row in reader:
        if row['type'] == 'mcq':
            # Store options as a JSON string for easy retrieval
            options = json.dumps({
        ...
        elif row['type'] == 'coding':
            cur.execute("""
                INSERT INTO coding_challenges (
                    job_id, title, description, difficulty, 
                    function_name, initial_code, test_input, expected_output
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                job_id, row['question_or_title'], row['option_a'], row['option_b'],
                row['option_c'], row['option_d'], row['correct'], row['extra_data']
            ))
...
def submit_final(job_id):
    ...
    # 2. Fetch Coding Challenge & Private Test Cases from Database
    con = sqlite3.connect("resume_screening.db")
    con.row_factory = sqlite3.Row
    cur = con.cursor()
...
def view_admin(admin_id):
    ...
    jobs = cur.execute("""
        SELECT *,
        (SELECT COUNT(*) FROM resumes WHERE resumes.job_id=jobs.id) AS applicants
        FROM jobs
        WHERE admin_id=?
    """, (admin_id,)).fetchall()

    con.close()
    return render_template("head_admin_jobs.html", jobs=jobs)
    ...
    try:
        if request.method == "POST":
            ...
            cur.execute("""
                ...
                mcq_completed, coding_completed)
                VALUES (?,?,?,?,?,?,?,?,?,0,0)
...
def download_report():
    ...
    SELECT 
        ...
        rh.match_score AS Coding_Score_Percent, 
...
@app.route('/run_public_tests/<int:job_id>', methods=['POST'])
def run_public_tests(job_id):
    data = request.json
    user_code = data.get('code')
    
    # Fetch ONLY public test cases from DB
    con = sqlite3.connect("resume_screening.db")
    con.row_factory = sqlite3.Row
    challenge = con.execute("SELECT * FROM coding_challenges WHERE job_id=?", (job_id,)).fetchone()
    con.close()

    passed, results = run_test_cases(user_code, "solution", challenge['public_test_cases'])
...
def grade_quiz(job_id):
    ...
    try:
        ...
        if percentage >= 80:
            ...
            flash(f"You scored {percentage}%. You need 80% to reach the coding round.", "error")
            return redirect(url_for('assessment_fail', job_id=job_id, score=percentage))
...
def job_status(job_id):
    ...
    app_data = con.execute("""
        SELECT mcq_score, mcq_completed, coding_completed 
        FROM resumes 
        WHERE user_id = ? AND job_id = ?
    ...
    status = {
        ...
        "coding_completed": bool(app_data['coding_completed']),
